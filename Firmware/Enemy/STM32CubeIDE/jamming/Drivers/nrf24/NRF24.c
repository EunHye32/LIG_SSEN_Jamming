...
void csn_high(void){
    if (active_nrf == 0) {
    	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET); // NRF_TX_CSN
    }else{
    	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_4, GPIO_PIN_SET); // NRF_RX_CSN
    }
}

void csn_low(void){
	if (active_nrf == 0) {
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_RESET); // NRF_TX_CSN
	}else{
		HAL_GPIO_WritePin(GPIOD, GPIO_PIN_4, GPIO_PIN_RESET); // NRF_RX_CSN
	}
}

void ce_high(void){
    if (active_nrf == 0) {
    	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_SET); // NRF_TX_CE
    }else{
    	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_5, GPIO_PIN_SET); // NRF_RX_CE
    }
}

void ce_low(void){
    if (active_nrf == 0) {
    	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET); // NRF_TX_CE
    }else{
